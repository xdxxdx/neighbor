		...
		table.on('tool(test)', function(obj) {
			...
			if (obj.event === 'detail') {
				...
				layer.confirm('真的删除行么', function(index) {
					$.ajax({
						type:'post',
						url:'./hidden?noticeId='+data.noticeId,
...
//				layer.alert('编辑行：<br>' + JSON.stringify(data))
				layui.use('layer', function() {
							...
							layer.open({
								...
								shade: 0.5,
								maxmin: true,
								content: './edit?noticeId='+data.noticeId
							});
						});
			}else if(obj.event=="show"){
				layer.confirm('真的设为显示吗', function(index) {
					$.ajax({
						type : 'post',
						url : './show?noticeId=' + data.noticeId,
						data : $('#thisForm').serialize(),
						dataType : 'html',
						success : function(data) {
							if (data > 0) {
				...
				layer.confirm('真的设为隐藏吗', function(index) {
					$.ajax({
						type : 'post',
						url : './unShow?noticeId=' + data.noticeId,
						data : $('#thisForm').serialize(),
						dataType : 'html',
						success : function(data) {
							if (data > 0) {

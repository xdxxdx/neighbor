...
mui('body').on('tap','.use_the_address',function(){
	...
	$.ajax({
		...
		success : function(data) {
			if(data > 0){
				mui.alert('完善地址成功\n正在返回我的订单','逸品生活',function(){location.href = '/wechatMyRealOrder'}); 
			}else{
...
mui('body').on('tap', '#toMyMall', function() {
...
function splitHtml(obj,ai) {
	...
	html = html + ' mui-radio mui-table-cell md_address pick-up-address"><div class="consignee_baseinfo"><span class="consignee_name">收件人：<em>'+obj.consignee;
	...
	html = html + '</em></div></div></div><div class="new_address_list_btools">'+footContent+'</div></div>';
	return html;
}

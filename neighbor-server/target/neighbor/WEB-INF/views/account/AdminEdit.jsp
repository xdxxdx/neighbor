...
<body class="hold-transition skin-red sidebar-mini">
			...
			<form class="form-horizontal" id="thisForm">
				...
				<div class="box-body">
					<div class="form-group">
						<label class="control-label label-layer-left" for="adminName">登录邮箱</label>
						<div class="div-layer-left">
							<input id="email" name="email" value="${account.email}" class="form-control"
								type="text" placeholder="登录邮箱">
						</div>
					</div>
					<div class="form-group">
						<label class="control-label label-layer-left" for="trueName">真实名称</label>
						<div class="div-layer-left">
							<input id="true" name="trueName" value="${account.trueName}" class="form-control"
								type="text" placeholder="请输入真实名称">
						</div>
					</div>
					<div class="form-group">
						<label class="control-label label-layer-left" for="roleId">所属小区</label>
						<div class="div-layer-left">
							<select name="communityId" class="form-control" >
								<option value="0" <c:if test="${account.communityId==0}">selected</c:if>>全部</option>
								<option value="7763" <c:if test="${account.communityId==7763}">selected</c:if>>陶然新村</option>
								<option value="7764" <c:if test="${account.communityId==7764}">selected</c:if>>东篱新村</option>
								<option value="7765" <c:if test="${account.communityId==7765}">selected</c:if>>陶然居</option>
							</select>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="div-layer-left">
							<select name="roleId" class="form-control" >
								<c:forEach items="${roleList}" var="role">
									<option value="${role.id}" <c:if test="${account.roleId==role.id}">selected</c:if>>${role.roleName}</option>
								</c:forEach>
							</select>
						</div>
					</div>
	...
	<!-- page script -->
	<script src="./static/js/back/admin/admin_add.js"></script>
</body>

			...
			if (obj.event === 'del') {
				...
				layui.use('layer', function() {
					...
					layer.open({
						...
						shade : 0.5,
						maxmin : true,
						content : './edit?advId=' + data.advId
					});
				});
			} else if (obj.event === 'show') {
				layer.confirm('真的设为显示？', function(index) {
					$.ajax({
						type : 'post',
						url : './setShow?advId=' + data.advId,
						data : $('#thisForm').serialize(),
						dataType : 'html',
						success : function(data) {
							if (data > 0) {
				...
				layer.confirm('真的设为隐藏？', function(index) {
					$.ajax({
						type : 'post',
						url : './setHidden?advId=' + data.advId,
						data : $('#thisForm').serialize(),
						dataType : 'html',
						success : function(data) {
							if (data > 0) {

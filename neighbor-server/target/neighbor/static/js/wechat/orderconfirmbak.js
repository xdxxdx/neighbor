...
$(function() {
	var price = $('.money_to_pay em').text();//就是$('#giveMoney').text();
	...
	document.getElementById('useScore').addEventListener('toggle', function(event) {
		...
		eActive ? $('#giveMoney').text((parseFloat($('#giveMoney').attr('yMoney')) - parseFloat($('#usedScore').val()) / 100).toFixed(2)) : $('#giveMoney').text($('#giveMoney').attr('yMoney'));
	...
	mui('.pay_footer').on('tap', '#confirm_to_save', function() {
		if ($('body').attr('bind') != 1 && $('body').attr('ismembercard') != '0') {

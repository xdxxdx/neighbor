...
mui('body').on('tap','.getcoupon',function(){
	if($('body').attr('bind') != '1'){
		var btnArray = ['否', '绑定'];
		mui.confirm('绑定逸品生活帐号，安全又实惠~', '逸品生活', btnArray, function(e) {
			if (e.index == 1) {
				location.href = '/wechatGoBind';
			}
		})
	...
	if($('body').attr('accountmode') == '1'){
		var btnArray = ['否', '切换'];
		mui.confirm('切换商城帐号，享受更多优惠', '逸品生活', btnArray, function(e) {
			if (e.index == 1) {
				$.ajax({
				      type : 'get',
				      ...
				      success : function(data) {
				    	  if(data == 1){
				    		  location.href = '/wechatMyShoppingCart?ts=' + Date.parse(new Date());
				    	  }else{
				    		  alert('切换帐号失败','逸品生活');
				    	  }
				      }
	...
	var couponJson = eval('(' + mParent.find('.couponJsonTemp').text()+ ')');
	...
	for(var i in couponJson.couponList){
		...
		var cHtml = '<li class="mui-table-view-cell" couponid="' + couponJson.couponList[i].couponId;
		...
		cHtml += '</p></div><div class="mui-table-cell mui-col-xs-2 mui-text-right getcoupon_area"><span class="getcoupon_btn">领取</span></div></div></li>';
...
$('#shopingcart_scroll').on('change','.shopingcart_number_box',function(){
	...
	$.ajax({
		type : 'get',
		url : '/updateShoppingCartCommodityNum?shoppingCartId=' + scid + '&commodityNum=' + num,
		dataType : 'html',
		success : function(data) {
	      ...
	      success : function(json) {
	    	  ...
	    	  for(i in json.merchantList){
	    		  ...
	    		  for(j in json.merchantList[i].shoppingCartList){
	    			  ...
	    			  si += '</div></div><div class="detele_shopingcart">删除</div></div><div class="couponJsonTemp"></div></div>';
	    		  }
	    		  si += '</div></div>';
...
function priceCalculation(){
	...
	$('input[name=realCommodityId]').each(function(){
		if($(this).prop('checked')){
			priceNum += 1;
			priceSum -= $(this).data('promotionprice') * $(this).attr('data-num');
		}
	});
	var obj = new Object();
	obj.sum = Math.abs(priceSum);
...
function getcoupon(){
	$('.shopingcart_item').each(function(){
		...
		$.ajax({
		      type : 'get',
		      url : '/usableCoupon?merchantId=' + merchantid,
		      dataType : 'json',
		      success : function(json) {
